...
void cs1237_task_handle(void)
{
    if(cs1237_comps.sw._bit.adc_updated)
    {
        ...
        {
            cs1237_comps.write_reg(&device_comps.temp_p_convert_result[device_comps.temp_p_pos++],0x4c);//10hze
        }
        else
        {
             cs1237_comps.write_reg(&device_comps.temp_p_convert_result[device_comps.temp_p_pos++],0x5c);//40hz
        }
...
void cs123x_task_handle(void)
{
    if(cs123x_comps.sw._bit.adc_updated)
    {
        if(cs123x_comps.current_channel==0)
        {
            if(hum_comps.current_mode==EM_CAL_MODIFY_MODE)
            {
                cs123x_comps.write_reg(&device_comps.ad1_convert_result[device_comps.ad1_pos++],0x45);//75
            }
            else
            {
                cs123x_comps.write_reg(&device_comps.ad1_convert_result[device_comps.ad1_pos++],0x75);//75
            }
        ...
        else if(cs123x_comps.current_channel==1)
        {
            if(hum_comps.current_mode==EM_CAL_MODIFY_MODE)
            {
                cs123x_comps.write_reg(&device_comps.deltaP_p_convert_result[device_comps.deltaP_p_pos++],0x44);//74
            }
            else
            {
                cs123x_comps.write_reg(&device_comps.deltaP_p_convert_result[device_comps.deltaP_p_pos++],0x74);//74
            }

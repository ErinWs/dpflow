...
void cs1237_task_handle(void)
{
    if(cs1237_comps.sw._bit.adc_updated)
    {
        ...
        {
            cs1237_comps.write_reg(&device_comps.temp_p_convert_result[device_comps.temp_p_pos++],0x4c);//10hze G=128 4c
        }
        else
        {
             cs1237_comps.write_reg(&device_comps.temp_p_convert_result[device_comps.temp_p_pos++],0x5c);//40hz G=128 5c
        }
...
void cs123x_task_handle(void)
{
    if(cs123x_comps.sw._bit.adc_updated)
    {
        if(cs123x_comps.current_channel==0)
        {
            if(hum_comps.current_mode==EM_CAL_MODIFY_MODE)
            {
                cs123x_comps.write_reg(&device_comps.ad1_convert_result[device_comps.ad1_pos++],0x45);////10hze
            }
            else
            {
                cs123x_comps.write_reg(&device_comps.ad1_convert_result[device_comps.ad1_pos++],0x55);//  //40hze
            }
        ...
        else if(cs123x_comps.current_channel==1)
        {
            if(hum_comps.current_mode==EM_CAL_MODIFY_MODE)
            {
                cs123x_comps.write_reg(&device_comps.deltaP_p_convert_result[device_comps.deltaP_p_pos++],0x44);//10hz
            }
            else
            {
                cs123x_comps.write_reg(&device_comps.deltaP_p_convert_result[device_comps.deltaP_p_pos++],0x54);//40hz
            }
            cs123x_comps.current_channel=0;
            cs123x_comps.power_down();
            cs123x_comps.sw._bit.running=0;
            if(!cs1237_comps.sw._bit.running)
            {
                MD_SET_REF_3030C_OFF;
            }
        }

...
//                        return 3+buf[1];
                   ...
                   return 0;
                   break;
           case MD_DATA_ID_READ_MEASURE_INFO:                //measure dpf_info
...
static void  loraComps_task_handle(void)
{
    ...
    if(init_once)
    {
        ...
        if(loraMisc.yl701_info.cs!=Check_Sum_5A(&loraMisc.yl701_info, & loraMisc.yl701_info.cs-(unsigned char *)&loraMisc.yl701_info))
        {
            ...
            loraMisc.yl701_info.freq=((unsigned long)0x6c<<16)+((unsigned long)0x80<<8)+0x12;
            loraMisc.yl701_info.sf=0x0b;//1024

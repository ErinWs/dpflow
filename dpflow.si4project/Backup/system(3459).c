...
pfun const _s_task_handle=_1s_task_handle;
...
{
    if(!(loraComps.sw._bit.runing)&&(!ircComps.sw._bit.runing)&&(!modbusComps.sw._bit.runing))
    {
        STOP();
    }
}
...
static task_comps_t task_comps[]=//50ms
{
	...
	{"",&task_comps[2],0,5       ,20,     &_s_task_handle          ,'F'},
	{"",&task_comps[3],0,5       ,1 ,     &loraComps.task_50ms     ,'F'},
	{"",&task_comps[4],0,100     ,0 ,     &ircComps.task_handle    ,'T'},//fast exe
	{"",&task_comps[5],0,10      ,0 ,     &loraComps.task_handle   ,'T'},//fast exe
	{"",&task_comps[6],0,2       ,0 ,     &pwd_mode_task           ,'T'},//fast exe
	{"",&task_comps[7],0,22      ,0 ,     &modbusComps.task_handle ,'T'}//fast exe
	//...TODO......
};
...
void user_init(void)
{
    ...
    while(!systemComps.test_xt1_run_flag)
    {
        timer++;
        if(timer>30000)
        {

...
static float get_Sated_vapor_density_by_press(long press) 
{
    float press_abs;
    float density=0;
    long  LowTValue;
    ...
    press_abs=press/fpwr(device_comps.calibration_param.dot);
    if(device_comps.calibration_param.unit!=0)
    {
        press_abs=press_abs/1000;
    }
    press_abs=press_abs+0.1013;
    ...
    else if (press_abs>Sated_vapor_press_abs_table[Top])       
    {
       // Bottom=Top-1;
        //goto insert_calc;
        return density=Sated_vapor_density_table[Top];
...
static float get_Suped_vapor_density_by_PT(long temp,long press)
{
    float press_abs;
    float density=0;
    long  LowTValue;
...
static  long dpf_calc_current_flow(device_comps_t *const this)
{
	...
	while(Top-Bottom>1)
	{
        ...
        {
            k = device_comps.dpf_param.k[i]%10000000;
            goto  ret;
        }
    }
insert_calc:
    {
        ...
        k =(long)(
        ((long long) (delta_p-LowTValue)*(device_comps.dpf_param.k[Top]%10000000-device_comps.dpf_param.k[Bottom]%10000000))
        /(HighTValue - LowTValue)
        )
        +device_comps.dpf_param.k[Bottom]%10000000;
    }
ret:
    {
        float p;
        float T=device_comps.current_temp/10.;
        float P=device_comps.current_press/fpwr(device_comps.calibration_param.dot);
        float fdelta_p=device_comps.current_deltaP/fpwr(device_comps.deltaP_calibration_param.dot);
        if(device_comps.calibration_param.unit!=0)
        {
            P=P/1000;
        }
        ...
        if(device_comps.dpf_param.bT==0)//other jiezhi
        {
            if(device_comps.dpf_param.tE&&device_comps.dpf_param.pE)
            {
                ...
                float p20=device_comps.dpf_param.p20%10000000/fpwr(device_comps.dpf_param.p20/10000000%10);
                float PA=device_comps.dpf_param.PA%10000000/fpwr(device_comps.dpf_param.PA/10000000%10);
                p=p20*(273.15+20)*(P+PA)/((T+273.15)*0.10133);
               
            }
            else if(device_comps.dpf_param.tE)
            {
                float A1=device_comps.dpf_param.A1%10000000/fpwr(device_comps.dpf_param.A1/10000000%10);
                float A2=device_comps.dpf_param.A2%10000000/fpwr(device_comps.dpf_param.A2/10000000%10);
                if(device_comps.dpf_param.A1/100000000)
                {
                    A1=-A1;
                }
                if(device_comps.dpf_param.A2/100000000)
                {
                    A2=-A2;
                }
                p=A1+A2*T;
    	...
    	if(device_comps.dpf_param.bT==1)//1 Saturated steam,
        {
            ...
            {
                 p=get_Sated_vapor_density_by_press(this->current_press);
            }
            else
            {
                 p=device_comps.dpf_param.p%10000000/fpwr(device_comps.dpf_param.p/10000000%10);
        ...
        else if(device_comps.dpf_param.bT==1)//1 superheated steam,
        {
            if(device_comps.dpf_param.tE&&device_comps.dpf_param.pE)
            {
                p=get_Suped_vapor_density_by_PT(this->current_temp,this->current_press);
            }
            else
            {
                  p=device_comps.dpf_param.p%10000000/fpwr(device_comps.dpf_param.p/10000000%10);
        ...
        {
            float M=k/fpwr(device_comps.dpf_param.k[0]/10000000%10)*sqrt(p*fdelta_p);
            device_comps.dpf_param.Fd%=4;
            if(device_comps.dpf_param.Fd==0)
            {
                return flow=M;
            }
            if(device_comps.dpf_param.Fd==1)
            {
                return flow=M*10;
            }
            if(device_comps.dpf_param.Fd==2)
            {
                return flow=M*100;
            }
            if(device_comps.dpf_param.Fd==3)
            {
                return flow=M*1000;
            }
...
device_comps_t device_comps=
{
    ...
    {7}, //union sw;  e2prom fault +cs123x err +cs1237err

...
{   
    ...
    10000,10039,10078,10117,10156,10195,10234,10273,10312,10351,//0......10
    ...
    31038,31070,31102,31134,31167,31199,31231,31263,31295,31327,//590...600
	...
	if(this->do_init==0)//&&(!loraComps.sw._bit.runing))
	{
		if(this->count==5)//every 1s calc press and temperature
		{
           ...
           // this->current_temp=this->calc_temp(this);
			...
			{
				device_comps.sw._bit.delataP_over_range=1;
			}
			else
			{
                device_comps.sw._bit.delataP_over_range=0;
			}
        ...
        cs1237_comps.restart();
        NOP();NOP();NOP();NOP();NOP();NOP();NOP();NOP();
        while(MD_CS1237_DATA);
        cs123x_comps.read_adc(&this->deltaP_p_convert_result[this->deltaP_p_pos++]);

...
//i=16ma/(2^N)*code+4ma
static void calc_4_20ma_output( long flw,long lower,long upper)
{
	/****START calc 4-20ma*******
...
****************************/
	float lower_range=lower%10000000/fpwr(lower/10000000%10);
	float upper_range=upper%10000000/fpwr(upper/10000000%10);
	float flow=flw/fpwr(device_comps.dpf_param.Fd);
	{
		long code=0;//code=(I-4)/16*65536
	    long output_i=0;
		unsigned int  data=0;
		if(!device_comps.dpf_param._4_20ma_dir)
		{
		    output_i=(20000-4000)*(flow-lower_range)/(upper_range-lower_range)+4000;
		}
		else
		{
           output_i=(4000-20000)*(flow-lower_range)/(upper_range-lower_range)+20000;
		}
		output_i=output_i * (device_comps.coe.current/10000.);
...
//        }
        if(!device_comps.read_coe(&device_comps.coe,sizeof(device_comps.coe)))
        {
            ...
            if(device_comps.coe.current<7000||device_comps.coe.current>13000)
            {
                device_comps.coe.current=10000;
            }
		...
		if(this->count==5)//every 1s calc press and temperature
		{
           ...
           // this->current_temp=this->calc_temp(this);
            ...
            if(device_comps.deltaP_calibration_param.is_calibrated)
            {
                ...
                calc_4_20ma_output(this->current_flow,this->dpf_param._4_20ma_limit[0],this->dpf_param._4_20ma_limit[1]);
		#endif

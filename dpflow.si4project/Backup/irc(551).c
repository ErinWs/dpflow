...
//					break;
			...
			case MD_DATA_ID_READ_MEASURE_INFO:                //measure info
					...
					VerifyResult=0;
					break;
			case MD_DATA_ID_READ_DPF_INFO:                //measure info
					ircMisc.send_buf[i++]=(buf[9]|0x80);
					ircMisc.send_buf[i++]=0;//Length
					ircMisc.send_buf[i++]=buf[11];//dataID
					ircMisc.send_buf[i++]=buf[12];
					...
					ircMisc.send_buf[i++]=temp>>8;
					ircMisc.send_buf[i++]=temp;
		            ircMisc.send_buf[i++]=device_comps.dpf_param.Fd;

                    temp=device_comps.dpf_data.total*lpwr(device_comps.dpf_param.Td)+(long)(device_comps.dpf_total_dec_part*lpwr(device_comps.dpf_param.Td));
		            ircMisc.send_buf[i++]=temp>>24;
					ircMisc.send_buf[i++]=temp>>16;
					ircMisc.send_buf[i++]=temp>>8;
					ircMisc.send_buf[i++]=temp;
		            ircMisc.send_buf[i++]=device_comps.dpf_param.Td;
		            
                    ircMisc.send_buf[i++]=device_comps.dpf_param.Fu;  
		...
		switch(DataId)
	    {
		    case MD_DATA_ID_ENTER_CAL_MODE:                //enter calmode
                ...
                else 
                {
				   ...
				   {
                        device_comps.deltaP_calibration_param_bak.dot=buf[13]&0x0f;
                        device_comps.deltaP_calibration_param_bak.unit=buf[14];
				   }
			...
			case MD_DATA_ID_WRITE_CAL_DATA:       //write cal data       
    			...
    			else 
    			{
                   ...
                   else if(device_comps.cal_type==EM_CAL_DELTAP)
                   {
                        ...
                        {
                            case 0:
                                        device_comps.deltaP_calibration_param_bak.x[0]=device_comps.deltaP_p_deltaP_n_average_result;
                                        device_comps.deltaP_calibration_param_bak.y[0]=param;
                                        mode_comps[hum_comps.current_mode].dis_option++;
                                        hum_comps.dis_oper_mark._bit.refresh_option=1;
                            ...
                            case 1:
                                        device_comps.deltaP_calibration_param_bak.x[1]=device_comps.deltaP_p_deltaP_n_average_result;
                                        device_comps.deltaP_calibration_param_bak.y[1]=param;
                                        mode_comps[hum_comps.current_mode].dis_option++;
                                        hum_comps.dis_oper_mark._bit.refresh_option=1;
                                        hum_comps.dis_oper_mark._bit.refresh_deltaP_cal_param=1;
                                        break;
                            case 2:
                                        device_comps.deltaP_calibration_param_bak.x[2]=device_comps.deltaP_p_deltaP_n_average_result;
                                        device_comps.deltaP_calibration_param_bak.y[2]=param;
                                        mode_comps[hum_comps.current_mode].dis_option++;
                                        hum_comps.dis_oper_mark._bit.refresh_option=1;
                                        hum_comps.dis_oper_mark._bit.refresh_deltaP_cal_param=1;
                                        break;
                            case 3:     
                                        device_comps.deltaP_calibration_param_bak.x[3]=device_comps.deltaP_p_deltaP_n_average_result;
                                        device_comps.deltaP_calibration_param_bak.y[3]=param;
                                        device_comps.deltaP_calibration_param_bak.is_calibrated=1;
                                        device_comps.deltaP_calibration_param_bak.cs=Check_Sum_5A(&device_comps.deltaP_calibration_param_bak, & device_comps.deltaP_calibration_param_bak.cs-(unsigned char *)&device_comps.deltaP_calibration_param_bak);
                                        if(device_comps.save_deltaP_calibration_param(&device_comps.deltaP_calibration_param_bak,sizeof(device_comps.deltaP_calibration_param_bak)))
                        				{
                        				    ircMisc.send_buf[i]=(buf[9]|0x90);
                                            mode_comps[hum_comps.current_mode].dis_option=0;
                                            hum_comps.dis_oper_mark._bit.refresh_option=1;
                                            hum_comps.dis_oper_mark._bit.refresh_deltaP_cal_param=1;
                                            device_comps.deltaP_calibration_param_bak.is_calibrated=0;
                        				}
                        				else
                        				{
                        					memcpy(&device_comps.deltaP_calibration_param,&device_comps.deltaP_calibration_param_bak,sizeof(device_comps.deltaP_calibration_param_bak));
                        					ircMisc.send_buf[i]=(buf[9]|0x80);
                        					#if(MD_PRODUCT_NAME==MD_LORA)
                                    		  {
                                    			  enter_lora_mode();
                   ...
                   else if(device_comps.cal_type==EM_CAL_RES)
                   {
                        switch(buf[13])
                        {
                            ...
                            case 1:
                                        ...
                                        device_comps.res_calibration_param_bak.is_calibrated=1;

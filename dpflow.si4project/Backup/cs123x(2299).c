...
void cs1237_task_handle(void)
{
    ...
    {
        cs1237_comps.read_adc(&device_comps.temp_p_convert_result[device_comps.temp_p_pos++]);
...
void cs123x_task_handle(void)
{
    if(cs123x_comps.sw._bit.adc_updated)
    {
        ...
        {
            cs123x_comps.write_reg(&device_comps.ad1_convert_result[device_comps.ad1_pos++],0x71);//75
            cs123x_comps.current_channel++;
            cs123x_comps.enable_interrupt();
        }
        else if(cs123x_comps.current_channel==1)
        {
            cs123x_comps.write_reg(&device_comps.deltaP_p_convert_result[device_comps.deltaP_p_pos++],0x70);//74

...
#define   MD_CS1237_DRIVER_ERR           (1<<2)


#if defined (MD_USE_RTC_MODULE)
#define   MD_RTC_MODULE_ERR              (1<<3)
#else
#define   MD_RTC_MODULE_ERR              (0<<3)
#endif

#if(MD_PRODUCT_NAME==MD_LORA)
#define   MD_LORA_MODULE_ERR             (1<<4)
#else
#define   MD_LORA_MODULE_ERR             (0<<4)
#endif

#if defined (MD_USE_4_20MA_MODULE)
#define   MD_PCF857X_DRIVER_ERR           (1<<5)
#define   MD_AD54X0_DRIVER_ERR            (1<<6)
#else
#define   MD_PCF857X_DRIVER_ERR           (0<<5)
#define   MD_AD54X0_DRIVER_ERR            (0<<6)
#endif
...
#define MD_DPF_PARAM_START_ADDR                        0x0400

#define MD_DPF_DATA_START_ADDR                        0x0700
#define MD_DPF_BACKUP_DATA_START_ADDR                 0x07f8
...
typedef struct _DEVICE_COMPONENTS
{
    ...
    int (*read_dpf_param)(void *,int );
    int (*save_dpf_param)(void const *,int); 
    long current_flow;
    
    dpf_data_t dpf_data;
    int (*read_dpf_data)(void *,int );
    int (*save_dpf_data)(void const *,int);
    float dpf_total_dec_part;

    dpf_backup_data_t dpf_backup_data;
    int (*read_dpf_backup_data)(void *,int );
    int (*save_dpf_backup_data)(void const *,int); 
